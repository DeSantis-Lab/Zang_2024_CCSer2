// Modified by A.G. in 2020 Augest based on the previous script written by Breane Budaitis.
...
n_channels = 4; // set number of channels; default is 3 (usually enough)
setTool("rectangle");
...
run("Convert to Mask");
run("Analyze Particles...", "size=5000-50000 show=Nothing display exclude add");
...
File.makeDirectory(output+ "GolgiFinal"); // initiate a place to save the final results, which is the normalized distribution profile;
File.makeDirectory(output+ "MitoFinal");
saveAs("Results", output + "/" + "GolgiFinal" + "/"+ "Results.csv");
saveAs("Results", output + "/" + "MitoFinal" + "/"+ "Results.csv");
...
waitForUser("Select the GOLGI window to measure intensity profile");
...
if (factor >= 0) {
	...
	run("Flatten");
	saveAs("PNG", output + "Golgi Scanning lines.png");
	close();
	}
else {
	...
	roiManager("delete");
	roiManager("save", output + "ROI_snd.zip"); // safe the second ROI set;
//	run("Clear Results");
//	roiManager("Measure");
//	saveAs("Results", output + "Results.csv"); // save the measurement of mean intensity and area information of each ROI;
...
File.makeDirectory(output+ "GolgiProfile");
for (i = 0; i < roiManager("count"); i++) {
	run("Clear Results");
	roiManager("Select", i);
	profile = getProfile();
	...
	updateResults();
	File.makeDirectory(output + "/" + "GolgiProfile" + "/" + i);
	saveAs("Results", output + "/" + "GolgiProfile" + "/" + i + "/Results.csv");
	}
run("Clear Results");
...
/* Mitochondrial internsity, save to <GolgiFinal folder*/
...
for (i = 0; i < m; i++) {	// iterate through the profile of all the ROIs
	...
	open(output + "/" + "GolgiProfile" + "/" + i + "/Results.csv");
	...
	open(output + "/" + "GolgiFinal" + "/"+ "Results.csv"); //update the binned intensity profile of each ROI 
...
Totalintensity = 0;
open(output + "/" + "GolgiFinal" + "/"+ "Results.csv");
...
/* Mitochondrial internsity, save to MitoFinal folder*/


waitForUser("please select the MITOCHONDRIA window to measure intensity profile");
run("Subtract Background...", "rolling=50"); // local uneven illumination background subtraction by "rolling ball" algrithm
roiManager("reset");
open(output + "ROI.zip");
roiManager("Show All without labels");
run("Flatten");
saveAs("PNG", output + "Mito Scanning lines.png");
close();


File.makeDirectory(output+ "MitoProfile");
for (i = 0; i < roiManager("count"); i++) {
	run("Clear Results");
	roiManager("Select", i);
	profile = getProfile();
	...
	updateResults();
	File.makeDirectory(output + "/" + "MitoProfile" + "/" + i);
	saveAs("Results", output + "/" + "MitoProfile" + "/" + i + "/Results.csv");
	}
run("Clear Results");
...
setOption("ExpandableArrays", true);
BinvalueSumMito = newArray(BinNumber); //sum the intensity within a certain bin of all ROIs; get an array of summed bin values;

for (i = 0; i < m; i++) {	// iterate through the profile of all the ROIs
		
	open(output + "/" + "MitoProfile" + "/" + i + "/Results.csv");
	s = getValue("results.count");
	binsize = s/BinNumber;
	
	setOption("ExpandableArrays", true);
	BinvalueArrayMito = newArray(0);
	...
	BinvalueArrayMito[BinNumber-1] = LastBinvalue;
	setResult("Binvalue", BinNumber-1, LastBinvalue);
	updateResults();
	//print(BinValueArray[bin-1]);
	saveAs("Results", output + "/" + "MitoProfile" + "/" + i + "/Results.csv");
	...
	for (k = 0; k < BinNumber; k++) {
		BinvalueSumMito[k] = BinvalueSumMito[k] + BinvalueArrayMito[k]; //add up the Binvalue of the current ROI to the BinvalueSum array;
		}
	
	open(output + "/" + "MitoFinal" + "/"+ "Results.csv"); //update the binned intensity profile of each ROI 
	for (z = 0; z < BinNumber; z++) {
		setResult(i, z, BinvalueArray[z]);
	}
	updateResults();
...
Totalintensity = 0;
open(output + "/" + "MitoFinal" + "/"+ "Results.csv");
for (e = 0; e < BinNumber; e++) {
	Totalintensity = Totalintensity + BinvalueSumMito[e];
}

for (f = 0; f < BinNumber; f++) {
	setResult("BinvalueSum", f, BinvalueSumMito[f]/Totalintensity);
}
updateResults();
saveAs("Results", output + "/" + "MitoFinal" + "/"+ "Results.csv");
